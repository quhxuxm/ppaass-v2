...
impl RsaCrypto {
        ...
        B: Read + Debug,
    {
        ...
        let public_key = RsaPublicKey::from_public_key_pem(&public_key_string)
            .map_err(|e| CryptoError::Rsa(format!("Fail to create rsa public key: {e:?}")))?;
        ...
        let private_key = RsaPrivateKey::from_pkcs8_pem(&private_key_string)
            .map_err(|e| CryptoError::Rsa(format!("Fail to create rsa private key: {e:?}")))?;
        Ok(Self {
            public_key,
            private_key,
        })
    ...
    pub fn decrypt(&self, target: &[u8]) -> Result<Vec<u8>, CryptoError> {
        let result = self
            ...
            .map_err(|e| CryptoError::Rsa(format!("Fail to decrypt with rsa: {e:?}")))?;
        Ok(result)
    }
}
