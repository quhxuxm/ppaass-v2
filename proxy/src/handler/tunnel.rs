...
pub async fn tunnel_init(
    ...
    let tunnel_init_request = agent_connection_framed.next().await.ok_or(ProxyError::AgentTcpConnectionExhausted)??;
    let TunnelInitRequest {
        agent_encryption, auth_token, dst_address, tunnel_type
    } = tunnel_init_request;
    let destination_transport = match tunnel_type {
        TunnelType::Tcp => DestinationTransport::new_tcp(&dst_address, server_state.clone()).await?,
        TunnelType::Udp => DestinationTransport::new_udp(&dst_address, server_state.clone()).await?,
    };
    ...
    agent_connection_framed.send((auth_token.clone(), tunnel_init_response)).await?;
    let FramedParts { io: agent_tcp_stream, .. } = agent_connection_framed.into_parts();
    Ok(TunnelInitResult {
        agent_encryption,
        proxy_encryption,
