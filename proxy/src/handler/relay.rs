...
pub async fn start_relay(
    ...
    if let (Ok((_, destination_transport_tx)), Ok((_, mut agent_data_framed_tx))) = forward_result {
...
async fn forward_agent_to_destination(destination_address: &UnifiedAddress, mut agent_data_framed_rx: AgentDataPacketRead, mut destination_transport_tx: DestinationTransportWrite) -> Result<(AgentDataPacketRead, DestinationTransportWrite), ProxyError> {
    loop {
        match agent_data_framed_rx.next().await {
            None => {
                destination_transport_tx.flush().await?;
                return Ok((agent_data_framed_rx, destination_transport_tx));
            }
            Some(Err(e)) => {
            ...
            Some(Ok(agent_data_packet)) => {
                match agent_data_packet {
                    AgentDataPacket::Tcp(data) => {
                        destination_transport_tx.send(BytesMut::from_iter(data)).await?
                    }
                    AgentDataPacket::Udp { payload, .. } => {
                        destination_transport_tx.send(BytesMut::from_iter(payload)).await?
                    }
                }
            }
        };
...
async fn forward_destination_to_agent(destination_address: &UnifiedAddress, mut destination_transport_rx: DestinationTransportRead, mut agent_data_framed_tx: AgentDataPacketWrite) -> Result<(DestinationTransportRead, AgentDataPacketWrite), ProxyError> {
    loop {
        match destination_transport_rx.next().await {
            None => {
                agent_data_framed_tx.flush().await?;
                return Ok((destination_transport_rx, agent_data_framed_tx));
            }
            Some(Err(e)) => {
            ...
            Some(Ok(destination_data_packet)) => {
                match destination_data_packet {
                    DestinationDataPacket::Tcp(data) => {
                        agent_data_framed_tx.send(ProxyDataPacket::Tcp(data)).await?
                    }
                    DestinationDataPacket::Udp { data, destination_address } => {
                        agent_data_framed_tx.send(ProxyDataPacket::Udp {
                            payload: data,
                            destination_address,
                        }).await?
                    }
